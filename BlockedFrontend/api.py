...
class ApiClient(BaseApiClient):
    ...
    def reports(self, page, state=None, isp=None, category=None, reportercategory=None, list=None, year=None, policy=None, admin=False):
        ...
        if admin:
            req['admin'] = 1
        if state:
            assert state in ('open','sent','closed','rejected','cancelled','reviewed','featured','egregious','harmless','unresolved')
            req[state] = 1
        if category:
